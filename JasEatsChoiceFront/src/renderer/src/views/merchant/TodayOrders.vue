...
const orderOverview = computed(() => {
	...
	const pendingCount = filteredOrders.value.filter(
		(order) => order.status === "pending"
	).length;
	const preparingCount = filteredOrders.value.filter(
		(order) => order.status === "preparing"
	).length;
	const completedCount = filteredOrders.value.filter(
			...
			<div class="overview-info">
				...
				<div class="overview-stats">
					...
					<span class="stat-item"
						>💰 总金额：¥{{ orderOverview.totalAmount.toFixed(2) }}</span
					>
				</div>
				<div class="status-stats">
					<span class="stat-item"
						>🔴 待处理：{{ orderOverview.pendingCount }}</span
					>
					<span class="stat-item"
						>🟡 准备中：{{ orderOverview.preparingCount }}</span
					>
					<span class="stat-item"
						>✅ 已完成：{{ orderOverview.completedCount }}</span
					>
				</div>
			</div>
