		...
		.then((response) => {
			...
			if (response.data && response.data.code === "200") {
				...
				const processedDishes = response.data.data.map((dish) => {
					...
					if (dish.createTime) {
						dish.createTime = new Date(dish.createTime).toLocaleString(
							"zh-CN",
							{
								year: "numeric",
								month: "2-digit",
					...
					if (dish.updateTime) {
						dish.updateTime = new Date(dish.updateTime).toLocaleString(
							"zh-CN",
							{
								year: "numeric",
								month: "2-digit",
...
const saveEditedDish = () => {
	...
	axios
		.put(
			`${API_CONFIG.baseURL}${API_CONFIG.dish.detail}${requestData.id}`,
			requestData
		)
		.then((response) => {
			if (response.status === 200 && response.data && response.data.success) {
				...
				// 更新本地菜品列表
				const index = dishesList.value.findIndex(
					(item) => item.id === updatedDish.id
				);
				if (index !== -1) {
...
const updateDishStatusInMenu = (menu) => {
	axios
		.put(
			`${API_CONFIG.baseURL}/v1/menus/menu/${menu.id}/dishes/${currentDish.value.id}/status`,
			{
	...
	ElMessageBox.confirm(getConfirmMessage(), "操作确认", {
		...
		.then(() => {
			...
			if (operation === "online" || operation === "offline") {
				...
				// 批量删除操作
				dishesList.value = dishesList.value.filter(
					(dish) => !selectedDishes.value.includes(dish)
				);
				updateFilter();
				selectedDishes.value = [];
				ElMessage.success(getSuccessMessage());
...
const openEditDishDialog = (dish) => {
	...
	editDishForm.value = JSON.parse(
		JSON.stringify({
			...dishWithoutStatus,
			ingredients: {
				mandatory: Array.isArray(dish.ingredients?.mandatory)
					? dish.ingredients.mandatory
					: [],
				optional: Array.isArray(dish.ingredients?.optional)
	...
	() => {
		...
		if (selectedDishes.value.length > filteredDishes.value.length) {
			...
			selectedDishes.value = selectedDishes.value.filter((selectedDish) =>
				filteredDishes.value.some(
					(filteredDish) => filteredDish.id === selectedDish.id
				)
			);
		}
	}
		...
		<el-empty v-if="filteredDishes.length === 0" description="暂无菜品">
			<template #bottom>
				<el-button
					type="primary"
					size="small"
					@click="addDishDialogVisible = true"
					>新增菜品</el-button
				>
			</template>
		</el-empty>
				...
				>
					...
					<el-form-item label="必选食材">
						...
						<div class="optional-ingredients-container">
							...
							<div class="ingredients-tags">
								<el-tag
									v-for="(ingredient, index) in editDishForm.ingredients
										.mandatory"
									:key="index"
									type="warning"
									closable
		...
		>
			<div class="menu-association-container">
				...
				<div class="batch-operation">
					...
					<el-button type="primary" @click="batchUpdateDishStatus">
						批量应用到所有菜单
					</el-button>
				</div>
				...
				>
					...
					>
						<template #default="{ row }">
							<el-tag
								:type="
									row.menu_status === 'active' ? 'success' : 'danger'
								"
								size="small"
								effect="light"
							>
