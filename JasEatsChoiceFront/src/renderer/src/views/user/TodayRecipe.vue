	...
	if (selectedRecipe.value && selectedDish.value && selectedRecipe.value.items) {
		...
		if (index !== -1) {
			...
			axios
				.put(
					API_CONFIG.baseURL +
						API_CONFIG.recipe.update +
						selectedRecipe.value.id,
					updateData
				)
				.then((response) => {
					// 更新本地数据 - 确保items字段已解析
					const recipeIndex = todayRecipes.value.findIndex(
						(r) => r.id === selectedRecipe.value.id
					);
					if (recipeIndex !== -1) {
						// 确保返回的食谱有items数组并已解析
						const updatedRecipe = {
		...
		if (selectedRecipe.value) {
			...
			axios
				...
				.then((response) => {
					...
					ElMessage.success(
						`成功导入 ${selectedMerchantDishes.value.length} 道菜品`
					);
...
const confirmAddDish = () => {
	if (selectedRecipe.value && newDish.value.name.trim()) {
		...
		axios
			.put(
				API_CONFIG.baseURL + API_CONFIG.recipe.update + selectedRecipe.value.id,
				updateData
			)
			.then((response) => {
				// 更新本地数据 - 确保items字段已解析
		...
		.then((response) => {
			...
			updatedRecipes.forEach((updatedRecipe) => {
				...
				if (recipeIndex !== -1) {
					...
					const updatedRecipeWithParsedItems = Object.assign(
						{},
						originalRecipe,
						updatedRecipe,
						{
...
const filteredRecipes = computed(() => {
	...
	if (filters.value.mealType !== "all") {
		if (filters.value.mealType === "snack") {
			...
			filtered = filtered.filter(
				(recipe) => recipe && recipe.type === filters.value.mealType
			);
		}
	}
				...
				>
					...
					<div class="recipe-actions">
						...
						<el-button
							type="text"
							size="small"
							@click="openImportMerchantDish(recipe)"
							>导入商家菜品</el-button
						>
						<!-- 替换菜品按钮：仅在有菜品时显示 -->
						<el-dropdown
							trigger="click"
							v-if="recipe.items && recipe.items.length > 0"
						>
		...
		<div v-if="selectedRecipe" class="recipe-details">
			<div class="detail-item">
				...
				<span class="detail-value">
					{{
						selectedRecipe.type === "breakfast"
							? "早餐"
							: selectedRecipe.type === "lunch"
							? "午餐"
							: selectedRecipe.type === "dinner"
							? "晚餐"
							: selectedRecipe.type === "afternoon_tea" ||
							  selectedRecipe.type === "tea"
							? "下午茶"
							: selectedRecipe.type === "night_snack" ||
							  selectedRecipe.type === "snack"
							? "加餐"
							: selectedRecipe.type
					}}
				</span>
					...
					<template #default>
						<div>
							<div class="order-option-header">
								<span>{{
									`订单号: ${order.orderNo} - 总价: ${order.totalPrice}元`
								}}</span>
							</div>
