				...
				<div class="calendar-control">
					...
					>
						<template #suffix-icon>
							<el-icon class="calendar-suffix-icon"
								><CalendarIcon
							/></el-icon>
						</template>
					</el-date-picker>
				...
				>
					...
					<template #footer>
						<div class="dialog-footer">
							...
							<el-button type="primary" @click="submitAddRecordForm"
								>确定</el-button
							>
						</div>
					</template>
			...
			<div class="meal-sections">
				...
				<div v-if="getMealsByType('breakfast').length > 0" class="meal-section">
					...
					>
						...
						<el-icon
							:class="{ 'rotate-180': !expandedSections.breakfast }"
							class="arrow-icon"
						>
							<ArrowDown />
						</el-icon>
				...
				<div v-if="getMealsByType('lunch').length > 0" class="meal-section">
					...
					>
						...
						<el-icon
							:class="{ 'rotate-180': !expandedSections.lunch }"
							class="arrow-icon"
						>
							<ArrowDown />
						</el-icon>
				...
				<div v-if="getMealsByType('dinner').length > 0" class="meal-section">
					...
					>
						...
						<el-icon
							:class="{ 'rotate-180': !expandedSections.dinner }"
							class="arrow-icon"
						>
							<ArrowDown />
						</el-icon>
				...
				<div v-if="getMealsByType('snack').length > 0" class="meal-section">
					...
					>
						...
						<el-icon
							:class="{ 'rotate-180': !expandedSections.snack }"
							class="arrow-icon"
						>
							<ArrowDown />
						</el-icon>
...
const fetchDietRecords = async (date) => {
	try {
		...
		// 调用后端API
		const apiUrl =
			API_CONFIG.diet.date.replace("{userId}", userInfo.userId) + formattedDate;
...
const submitAddRecordForm = async () => {
	try {
		...
		const recordTime = new Date(
			`${selectedDate.value}T${addRecordForm.value.time}`
		).toISOString();
